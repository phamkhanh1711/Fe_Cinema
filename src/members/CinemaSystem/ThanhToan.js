...
function ThanhToan() {
  ...
  const handleDiscount = () => {
    ...
    axios
      .get(`http://localhost:4000/promotion/checkPromo`, {
        headers: {
      ...
      .catch((error) => {
        ...
        alert(
          error.response.data.message ||
            "Đã xảy ra lỗi khi áp dụng mã giảm giá. Vui lòng thử lại sau."
        );
      });
  };
  ...
  const renderMenu = () => {
    ...
    const totalSumDiscount = JSON.parse(
      localStorage.getItem("totalSumDiscount")
    ); // tổng giá sau khi giảm giá
  ...
  const renderPromotion = () => {
    return (
      <div className="apply-coupon-form">
        ...
        />
        <button onClick={handleDiscount} className="apply-coupon-submit button">
          Áp dụng
        </button>
      </div>
    );
